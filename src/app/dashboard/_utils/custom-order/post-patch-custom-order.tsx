...
import {
  T__Schema__CustomOrder,
  __Schema__CustomOrder,
} from "./custom-order.column";
...
import { A__PATCH__CustomOrder, A__POST__CustomOrder } from "./actions";

const PromoForm = ({
  defaultValues,
}: {
  defaultValues?: T__Schema__CustomOrder;
}) => {
  const router = useRouter();
  if (defaultValues?.expireDate) {
    defaultValues.expireDate = new Date(defaultValues.expireDate);
  }
  const form = useForm<T__Schema__CustomOrder>({
    resolver: zodResolver(__Schema__CustomOrder),
    defaultValues: defaultValues || Promo__DefaultValues,
  });

  async function onSubmit(data: T__Schema__CustomOrder) {
    ...
    const result = !defaultValues?.id
      ? await A__POST__CustomOrder(data)
      : await A__PATCH__CustomOrder(data);

    ResponseX({ title: "Custom Order", result });
    if (result.success) {
      router.push("/dashboard/custom-order");
    }
  }
    ...
    <Form {...form}>
      ...
      >
        ...
        <div className="flex items-center justify-end gap-4">
          <Link href="/dashboard/custom-order">
...
const Promo__DefaultValues: T__Schema__CustomOrder = {
  couponCode: "",
  discountType: "FIXED",
  discountAmount: 0,
  minOrderAmount: 0,
